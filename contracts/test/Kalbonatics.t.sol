...
contract KalbonaticsTest is Test {
    Registry registry;
    ...
    function testBet() public {
        kalboToken.approve(address(treasury), type(uint256).max);
        kalboToken.approve(address(kalbonatics), type(uint256).max);
        kalbonatics.bet(1 ether, "HEADS");
    }
}
